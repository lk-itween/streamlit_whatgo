...
def load_data():
    ...
    data_list = map(lambda x: pd.DataFrame(x.values, columns=['Food Items', 'Kcal per 100g']),
                    pd.read_html(resp.text, header=0))

    data = pd.concat(data_list, keys=['Fruits', 'Vegetables', 'Grains and Pulses', 'Meat', 'Fish', 'Dairy and Eggs',
                                      'Carbohydrates', 'Cooking Oils', 'Soft Beverages', 'Alcoholic Beverages'])
...
caloris_plt = ['Fruits', 'Vegetables', 'Grains and Pulses', 'Meat', 'Fish', 'Dairy and Eggs', 'Carbohydrates',
               'Cooking Oils', 'Soft Beverages', 'Alcoholic Beverages']
